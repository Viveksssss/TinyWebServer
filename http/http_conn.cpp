...
const char *error_400_form =
    "Your request has bad syntax or is inherently impossible to staisty.\n";
...
void http_conn::initmysql_result(connection_pool *connPool) {
  ...
  if (mysql_query(mysql,
                  "select username,passwd from username,passwd from user")) {
...
int setnonblocking(int fd) {
  ...
  fcntl(fd, O_SETFL, new_option);
  return old_option;
}

/* 内核时间注册，ET模式，选择开启EPOLLONSHOT */
...
http_conn::HTTP_CODE http_conn::process_read() {
  ...
  char *text = 0;
  while ((m_check_state == CHECK_STATE::CHECK_STATE_CONTENT &&
...
bool http_conn::write() {
  int temp = 0;
  if (bytes_to_send = 0) {
    modfd(m_pollfd, m_sockfd, EPOLLIN, m_TRIGMode);
    init();
    return true;
  }
  while (1) {
    ...
    if (bytes_have_send >= m_iv[0].iov_len) {
      /*
         ...
         100           120                         600
         ↑             ↑                            ↑
         m_file_address  m_iv[1].iov_base更新点     文件结束
       * */
      m_iv[0].iov_len = 0;
      ...
      /*
         ...
         0        60              100                    600
         ↑        ↑               ↑                       ↑
         m_write_buf  m_iv[0].iov_base更新点  文件起始地址
...
*/
    ...
    /*
       ...
       部分发送头（bytes_have_send=60）：
       [ Sent ][ Remaining ][ File Data ]
       |------|-----------|-------------|
       0     60         100           600

       头发完，部分文件（bytes_have_send=120）：
       [ Headers Done ][ Sent ][ Remaining File ]
       |--------------|------|------------------|
       0            100    120                 600
     * */
    if (bytes_to_send <= 0) {
...
bool http_conn::add_response(const char *format, ...) {
  ...
  if (len >= (WRITE_BUFFER_SIZE - 1 - m_wriet_idx)) {
    va_end(arg_list);
    return false;
  }
  m_write_idx += len;
